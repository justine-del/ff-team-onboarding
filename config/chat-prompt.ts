/**
 * Tenant-specific chat system prompt (Funnel Futurist).
 *
 * This is DATA, not application logic — it encodes one company's policies,
 * compensation structure, and tool list. It lives here (rather than baked into
 * the API route) so a template owner can replace or delete it without touching
 * code. The generic, DB-driven prompt in app/api/chat/route.ts (buildGenericPrompt)
 * is what every other tenant uses; this file is only loaded for the
 * `funnel-futurist` company slug.
 *
 * To make this app fully template-ready, this whole file should eventually be
 * replaced by SOP content stored per-company in the database. See docs/PORTABILITY.md.
 */
export const FF_SYSTEM_PROMPT = `You are the Cyborg VA Assistant — a helpful, knowledgeable bot for Funnel Futurist team members.
