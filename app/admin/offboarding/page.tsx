...
type VA = {
  ...
  job_role: string | null; start_date: string | null
}
...
const emptyForm = {
  ...
  ghl_removed: false, notion_removed: false, geekbot_removed: false, other_tools: '',
  handoff_tasks: '', handoff_docs: '', replacement_needed: false, final_notes: '',
}
...
export default function OffboardingPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from('profiles').select('*').eq('role', 'member').order('first_name')
      setVAs(data ?? [])
    }
    load()
  }, [router])
  ...
  const sectionHead = 'text-sm font-semibold text-gray-300 mb-4 uppercase tracking-wide'
        ...
        <div className="no-print bg-gray-900 text-white px-6 py-3 flex items-center gap-4 sticky top-0 z-10">
          ...
          {!disabled ? (
            <button onClick={handleDisable} disabled={disabling} className="text-sm bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg ml-auto">
              {disabling ? 'Disabling...' : 'Disable Login'}
            </button>
          ) : (
            <span className="text-green-400 text-sm ml-auto">✓ Login disabled</span>
          )}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-8 py-10 bg-white shadow-lg my-6">
          ...
          <div className="border-b-2 border-teal-700 pb-4 mb-6">
            ...
            <div className="mt-3 flex flex-wrap gap-6 text-sm text-gray-600">
              ...
              <span><strong>Status:</strong> {disabled ? '🔴 Login Disabled' : '🟡 Login Active'}</span>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        {selectedVA && (
          <div className="flex items-center gap-3">
            {!disabled ? (
              ...
              <span className="text-green-400 text-sm">✓ Login disabled</span>
            )}
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className={cardClass}>
          <p className={sectionHead}>Select VA to Offboard</p>
          <select
          ...
          >
            ...
            {vas.map(va => (
              <option key={va.id} value={va.id}>{va.first_name} {va.last_name} ({va.email})</option>
            ))}
          </select>
        </div>
        ...
        {selectedVA && (
          <>
            {/* 1. VA Info */}
            <div className={cardClass}>
              <p className={sectionHead}>1. VA Information</p>
              ...
              <div className="mt-4">
                ...
                <textarea className={inputClass} rows={3} value={form.offboard_notes} onChange={e => set('offboard_notes', e.target.value)} placeholder="Context, red flags, additional details..." />
              </div>
            </div>

            {/* 2. KPI Review */}
            <div className={cardClass}>
              <p className={sectionHead}>2. KPI & Work Review</p>
              {kpiLoading ? (
            ...
            {/* 3. Invoice */}
            <div className={cardClass}>
              <p className={sectionHead}>3. Final Invoice</p>
              ...
              <textarea className={inputClass} rows={2} value={form.invoice_notes} onChange={e => set('invoice_notes', e.target.value)} placeholder="Payment adjustments, based on deliverables..." />
            </div>

            {/* 4. Tool Access */}
            <div className={cardClass}>
              <p className={sectionHead}>4. Tool Access Revocation</p>
              ...
              <input className={inputClass} value={form.other_tools} onChange={e => set('other_tools', e.target.value)} placeholder="1Password, ClickUp, etc." />
            </div>

            {/* 5. Handoff */}
            <div className={cardClass}>
              <p className={sectionHead}>5. Handoff & Close Out</p>
              <div className="mb-4">
            ...
            <div className="flex justify-end gap-3 pb-8">
              {!disabled ? (
                ...
                <span className="text-green-400 text-sm flex items-center">✓ Login disabled</span>
              )}
